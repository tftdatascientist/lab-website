...
export function Wordmark({ progress, color, scale = 1 }: { progress: number; color: string; scale?: number }) {
  const full = 'lok-ai';
...
export function UrlLine({ progress, color }: { progress: number; color: string }) {
...
export function Tagline({ progress, color }: { progress: number; color: string }) {
  const ease = Easing.easeOutCubic(progress);
  return (
    <div style={{
      position: 'absolute',
...
export function StatusBar({
  ...
  const c = operational ? secondary : color;
  return (
    <div style={{
      position: 'absolute',
      top: 72,
      ...
      <span style={{ filter: `drop-shadow(0 0 4px ${c}88)` }}>{label}</span>
    </div>
  );
}

export function CornerReadout({
  progress, color, lines, position = 'bl',
}: {
  progress: number;
  color: string;
  lines: string[];
  ...
  const ease = Easing.easeOutCubic(progress);
  const pos = position === 'bl' ? { left: 72, bottom: 72 } : { right: 72, bottom: 72 };
  return (
    <div style={{
      position: 'absolute',
      ...pos,
...
export function Crosshair({
  x, y, progress, color, size = 60,
}: {
  x: number; y: number; progress: number; color: string; size?: number;
}) {
  const ease = Easing.easeOutCubic(progress);
...
export function AmbientBackdrop({ color, intensity = 1 }: { color: string; intensity?: number }) {
  return (
    <>
      <div style={{
      ...
      <div style={{
        ...
        background: 'radial-gradient(circle at 50% 50%, rgba(0,0,0,0) 0%, rgba(0,0,0,0.55) 100%)',
        pointerEvents: 'none',
      }} />
    </>
  );
...
export function Starfield({ pulseT, color, opacity = 1 }: { pulseT: number; color: string; opacity?: number }) {
  return (
    <svg viewBox="0 0 1920 1080" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity }}>
      {STARS.map((s, i) => {
        ...
        return (
          <circle key={i} cx={s.x} cy={s.y} r={s.size} fill={color} opacity={0.15 + 0.35 * tw} />
        );
      })}
    </svg>
  );
...
export function CornerFrames({ color, glow }: { color: string; glow: number }) {
  const f = glow > 0 ? `drop-shadow(0 0 4px ${color}88)` : 'none';
  const stroke: React.SVGProps<SVGPathElement> = {
    stroke: color, strokeWidth: 1.5, fill: 'none', strokeOpacity: 0.55,
  };
  return (
