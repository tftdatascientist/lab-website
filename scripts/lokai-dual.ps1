...
#   powershell -ExecutionPolicy Bypass -File scripts\lokai-dual.ps1            # oba
#   powershell -ExecutionPolicy Bypass -File scripts\lokai-dual.ps1 -TylkoNowy # tylko 3301 (do kodowania ETAP2)
#   ... -BezPrzegladarki                                                        # nie otwieraj kart (skrypty, testy)
param(
    [switch]$TylkoNowy,
...
# Skrypt może leżeć w obu katalogach (main i worktree) — katalogi rozstrzyga nazwa, nie położenie skryptu.
$parent = Split-Path -Parent $root
$old  = Join-Path $parent "lab-website"
$new  = Join-Path $parent "lab-website-redesign"
...
if (-not $TylkoNowy) { Start-Dev $old 3300 "STARY" }
Start-Dev $new 3301 "NOWY"

# czekaj (maks. 90 s), potem otworz karty
$deadline = (Get-Date).AddSeconds(90)
if ($TylkoNowy) {
    while ((Get-Date) -lt $deadline -and -not (Test-Port 3301)) { Start-Sleep -Milliseconds 800 }
    if (-not $BezPrzegladarki) { Start-Process "http://localhost:3301/" }
    Write-Host "NOWY: http://localhost:3301"
} else {
    while ((Get-Date) -lt $deadline -and -not ((Test-Port 3300) -and (Test-Port 3301))) { Start-Sleep -Milliseconds 800 }
    if (-not $BezPrzegladarki) { Start-Process "http://localhost:3300/"; Start-Process "http://localhost:3301/" }
